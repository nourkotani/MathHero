...
own bones. The brief's "score" weighs the measures. Writes
...
The measures, in the hero's space (it faces the Dummy along Blender -Y):
  ...
  hunch    most the torso bends from upright (rad)
...
def measure(rig, action, drift):
    ...
    reach = back = air = hunch = turn = travel = 0.0
    last = None
    for f in range(start, end + 1):
        ...
        hunch = max(hunch, (torso @ Vector((0.0, 1.0, 0.0))).angle(UP))
...
def main():
    ...
    for spec in specs:
        ...
        score = sum(weight * metrics[k] for k, weight in brief["score"].items())
