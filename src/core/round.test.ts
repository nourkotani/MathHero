...
describe('timer settings', () => {
  it('defaults to two minutes', () => {
    expect(initialState({ seed: 1 }).timerSeconds).toBe(DEFAULT_TIMER_SECONDS);
  ...
  it('cannot be changed mid-Round', () => {
    const inRound = update(initialState({ seed: 1 }), { type: 'ROUND_STARTED' }).state;
...
describe('Round lifecycle', () => {
  it('starting a Round resets the score and asks a question', () => {
    const state = initialState({ seed: 1 });
    const result = update(state, { type: 'ROUND_STARTED' });
  ...
  it('remaining time is derived from the round start and the latest tick', () => {
    let state = dispatchAll(initialState({ seed: 1 }), [
      { type: 'TICK', now: 1000 },
      { type: 'ROUND_STARTED' },
    ]);
  ...
  it('a tick past the deadline ends the Round instantly and voids the typed answer', () => {
    const inRound = dispatchAll(initialState({ seed: 1 }), [
      { type: 'TICK', now: 0 },
      { type: 'ROUND_STARTED' },
    ...
    expect(result.state.score).toBe(0);
    expect(result.effects).toEqual([{ type: 'ROUND_ENDED', finalScore: 0 }]);
  });

  it('the ended Round keeps its final score for the Results screen', () => {
    let state = dispatchAll(initialState({ seed: 5 }), [
      { type: 'TICK', now: 0 },
      { type: 'ROUND_STARTED' },
    ]);
