...
// frame and back. The director still hears the cue's own name at its end.
...
/** Cues with no clip of their own, and the clip that plays for each. */
const STAND_IN: Readonly<Record<string, string>> = { Launch: 'HitSpin', Recover: 'HitBack' };
const clipFor = (cue: string) => STAND_IN[cue] ?? cue;

/** The launch: how far up and back the fighter flies (out of the frame). */
const LAUNCH = { back: 7, up: 3, drop: 0.5 };
...
export function Rival({
  ...
  useEffect(() => {
    ...
    const onFinished = (event: { action: THREE.AnimationAction }) => {
      ...
      if (clipFor(now) === event.action.getClip().name) director.finished(now);
    };
  ...
  useFrame((_, delta) => {
    ...
    if (!group) return;
    const now = cueRef.current.clip;
    const length = actions[clipFor(now)]?.getClip().duration ?? 1;
    if (now === 'Launch') {
      const t = Math.min(1, since.current / length);
      group.position.set(LAUNCH.back * easeIn(t), LAUNCH.up * easeOut(t), 0);
    } else if (now === 'Recover') {
      const t = 1 - easeOut(Math.min(1, since.current / LAUNCH.drop));
