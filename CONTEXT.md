...
A self-contained, offline 3D multiplication practice game (times tables 1–12) for kids, with DBZ-inspired power-up spectacle.
...
### Round
One timed play session: a Player answers Questions until the timer expires. Configured by a Difficulty and a timer length (default 2 minutes, settable 30 seconds–10 minutes). Produces a score.

### Question
A single multiplication prompt (e.g., "7 × 8"). Answered by typing the number (on-screen number pad or keyboard) and **explicitly submitting** via Enter or a big ✓ button (backspace allowed; the ✓ doubles as the "fire the blast" trigger). A wrong answer costs no points, breaks the Power Streak, and briefly shows the correct answer before the next Question. When the Round timer hits zero, the Round ends instantly and any in-progress Question is voided. The countdown is always visible and pulses during the final 10 seconds.

### Fact
A commutative multiplication pair treated as a single unit of mastery: 7×8 and 8×7 are **one Fact**, displayed in either order at random. Tables 1–12 therefore contain 78 Facts.

### Difficulty
Determines which times tables are in play and the base points per correct answer. "Tables N–M" means **one operand comes from N–M and the other from 1–12** (the way school teaches a times table):
...
### Adaptive Selection
Question picking is weighted random: Facts the Player answers wrongly or slowly appear more often until mastered. The same question never appears twice in a row. A Fact is **mastered** when its last 3 answers were all correct and each took under ~6 seconds; a wrong answer un-masters it.

### Mastery Grid
A per-Player 12×12 times-table chart on the Title screen, coloring each Fact green/yellow/red by mastery state — a progress report for the parent, driven by the same data Adaptive Selection uses.
...
### Personal Best
A Player's highest Round score, tracked separately per Difficulty. Beating one triggers its own celebration.

### Family Leaderboard
Cross-Player comparison of Personal Bests, shown on the main menu.
...
### Screens
Title (player select, Family Leaderboard, New Hero, settings: export/import + sound toggle) → Hero creation (new players only) → Pre-round (difficulty + timer + Start) → Round (3D arena) → Results (score, best streak, XP gained, level-up ceremony, personal-best celebration) → back to Title.
